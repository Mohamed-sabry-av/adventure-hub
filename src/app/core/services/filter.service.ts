...
export class FilterService {
      ...
      this.wooAPI
        ...
        .pipe(
          map((response: HttpResponse<any>) => {
            console.log(
              'API response received for category',
              categoryId,
              response
            );
   ...
   */
  processAttributes(response: HttpResponse<any>): {
    attributes: {
      [key: string]: { name: string; terms: { id: number; name: string }[] };
    };
    totalPages: number;
    ...
    const attributesMap = new Map<
      string,
      { name: string; terms: Map<number, { id: number; name: string }> }
    >();
          ...
          attr.options.forEach((option: { id: number; name: string }) => {
            if (option.id && option.name) {
              ...
              if (!attributesMap.get(attrSlug)!.terms.has(termId)) {
                attributesMap
                  .get(attrSlug)!
                  .terms.set(termId, { id: termId, name: option.name });
              }
            }
          });
        }
   ...
   */
  getAllAttributesAndTermsByCategory(categoryId: number): Observable<{
    [key: string]: { name: string; terms: { id: number; name: string }[] };
  }> {
  ...
  private buildFilterParams(
    ...
    let params = new HttpParams()
      ...
      .set(
        '_fields',
        'id,name,price,images,categories,quantity_limits,description,sale_price,regular_price,on_sale,variations,currency,attributes'
      );
