...
export class CheckoutFormComponent {
  ...
  ngOnInit() {
    ...
    const subscription3 = this.billingForm.get('shippingMethod')?.valueChanges.subscribe((method) => {
      if (method === 'different') {
    ...
    const subscription = this.billingForm.get('paymentMethod')?.valueChanges.subscribe((method) => {
      this.toggleCreditCardValidators(method!);
    });

    const subscription2 = this.billingForm.get('countrySelect')?.valueChanges.subscribe((value) => {
    ...
    const subscription4 = this.shippingForm?.get('countrySelect')?.valueChanges.subscribe((value) => {
      if (this.isShippingDifferent) {
        this.onGetSelectedShippingCountry(value);
      }
    });
  ...
  async initWalletPayments() {
    ...
    try {
      ...
      if (result) {
        ...
        this.paymentRequest.on('paymentmethod', async (event: any) => {
          ...
          if (fullName) {
            ...
            lastName = '';
            console.warn('No name provided by Google Pay, using fallback:', firstName);
          }
          ...
          const shippingFullName = shippingAddress.recipient || fullName || billingDetails.email?.split('@')[0] || 'Customer';
          ...
          if (shippingFullName) {
            ...
            shippingLastName = shippingNameParts.length > 1 ? shippingNameParts.slice(1).join(' ') : '';
          }
          ...
          if (this.isShippingDifferent) {
            ...
            console.log('Shipping form after patching:', this.shippingForm.value);
          }
          ...
          try {
            ...
            if (paymentIntent && paymentIntent.status === 'succeeded') {
              ...
              try {
                ...
                this.uiService.showError('Failed to create order: ' + (orderError.message || 'Please try again'));
                event.complete('fail');
                return;
              }
            } else {
        ...
        this.paymentRequest.on('shippingaddresschange', (event: any) => {
          ...
          const shippingDetail = country === 'AE'
      ...
      this.uiService.showError('Failed to initialize wallet payments: ' + error.message);
    }
  }
    ...
    try {
      ...
      const { error, paymentIntent } = await this.stripe.confirmCardPayment(clientSecret, {
        payment_method: {
          ...
          billing_details: {
            name: `${this.billingForm.get('firstName')?.value} ${this.billingForm.get('lastName')?.value || ''}`.trim(),
  ...
  prepareOrderData({
    ...
    const billingFirstName = billingForm.get('firstName')?.value || billingForm.get('email')?.value?.split('@')[0] || 'Customer';
