...
export class CheckoutFormComponent {
  ...
  // Add property to store payment token
  ...
  async payNow() {
    if (this.paymentRadiosValue === 'stripe' && this.paymentToken) {
      // Use existing token if already generated by the payment component
      this.onSubmit(this.paymentToken);
    } else {
      console.error('Payment component did not generate a token');
      alert('Please complete the payment form first');
    }
  }

  // Handle payment token from payment component
  onPaymentTokenGenerated(token: string) {
    console.log('Payment token received:', token);
    this.paymentToken = token;

    // Auto-submit the form when a payment token is generated
    if (this.isFormValid()) {
      this.onSubmit(token);
    }
  }

  // Handle payment type selection from payment component
  onPaymentTypeSelected(paymentType: string) {
    console.log('Payment type selected:', paymentType);
    const paymentMethodControl = this.billingForm.get('paymentMethod');
    if (paymentMethodControl) {
      paymentMethodControl.patchValue(paymentType);
      this.toggleCreditCardValidators(paymentType);
    }
  }
